...
if not has_flag("百姓起义") then
    ...
    local nanzi = actor("[speaker:村民][img:npc_YuMing_2]")
...
if has_flag("完成变强之路") and has_flag("第一次收复") then
    ...
    --战斗
    local ret = start_preset_battle("虎苇地收复战2")
    if ret == 1 then
        situ:emotion("NaoHuo")
...
situ:say("六哥，这里的一草一木都是我小时候的回忆。我们将成为它的主人！")
tangzhu:say("你们这群丧家之犬，还敢回来？")

hide_all_stage_bubble()

--战斗
local ret = start_preset_battle("虎苇地收复战")
if ret == 1 then
    tangzhu:say("哼，算你好运，留得青山在，不怕没柴烧！")
    hide_all_stage_bubble()
    tangzhu:flip()
    tangzhu:movetoX(3000)
