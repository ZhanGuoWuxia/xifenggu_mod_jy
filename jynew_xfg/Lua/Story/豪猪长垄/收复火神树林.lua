...
if not has_flag("百姓起义") then
    ...
    local nanzi = actor("[speaker:村民][img:pawn_CunMing_male_2]")
    ...
    nanzi:say("掌门！您为我们做了这许多好事，如今也是我们报答的时候。现下城门已开！就等您一声号令！")
    for i = 1, 2 do
...
if has_flag("完成变强之路") and has_flag("第一次收复") then
    ...
    --战斗
    local ret = start_preset_battle("火神树林收复战2")
    if ret == 1 then
        situ:emotion("NaoHuo")
...
situ:say("六哥，这里就是虎焰山的火神树林了。")
qiliuge:say("掌门，这里的豪猪长垄弟子都是精锐，可要小心呀！")
tangzhu:say("哈哈哈，你们是来送死的吗？我等你们很久了！")

hide_all_stage_bubble()

--战斗
local ret = start_preset_battle("火神树林收复战")
if ret == 1 then
    tangzhu:say("可恶！竟然败在你们这群乡巴佬手里！")
    hide_all_stage_bubble()
    tangzhu:flip()
    tangzhu:movetoX(3000)
