local buff = buff_define("右势猴形")
--施展[左势猴形]后追击，且此招式无法被招架和闪避，也无法造成暴击。并根据双方速度差值提升此次伤害，每相差10点提升1%,最高50%

--在buff携带者（攻击方）命中判定前
function buff:BeforeAttackerHit(args)
    if not args.Skill:IsSkill("右势猴形") then
        return
    end
...
function buff:BeforeGiveDamage(args)
    if not args.DamageInfo.SourceSkillId == "右势猴形" then
        return
    end
...
function buff:AfterSkillCast(args)
    if not args.Skill:IsSkill("左势猴形") then
        return
    end

    local ownerId = self:GetBuffOwnerId(args)
    do_persue_skill(ownerId, "右势猴形", args.TargetParam.CastPosition , enemy_team_of_role(ownerId))
end
