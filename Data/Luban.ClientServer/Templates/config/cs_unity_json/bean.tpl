...
/// </summary>
{{~end~}}
public {{x.cs_class_modifier}} partial class {{name}} : {{if parent_def_type}} {{parent}} {{else}} {{x.namespace_with_top_module}}.BaseBean {{end}}
{
{{~if x.hierarchy_export_fields.size > 0 ~}}
    public {{name}}(){}
{{~end~}}
    ...
    {{~if x.is_abstract_type~}}
        ...
        {{~end~}}
            default: throw new SerializationException($"CannotLoad type: {type}");
        }
    {{~else~}}
...
{{~end~}}
    ...
    public {{cs_define_text_key_field field}} { get; set;}
    {{~end~}}
    {{~end~}}
...
{{~if !x.is_abstract_type~}}
    ...
    public override string GetPrimaryKey() => {{export_fields[0].convention_name}}.ToString();
{{~end~}}
