...
public sealed partial class {{name}}: {{~if x.is_map_table ~}} ITable<{{cs_define_type key_type}}, {{cs_define_type value_type}}> {{~end~}}
{
    ...
    public int BeanCount => _dataList.Count;

    public IEnumerable<BaseBean> GetAllBeans()
    {
        foreach(var bean in _dataList)
            yield return bean;
    }

    public void Merge(IEnumerable<BaseBean> newBeans)
    {
        Merge(newBeans.OfType<{{cs_define_type value_type}}>());
    }

    public void Merge(IEnumerable<{{cs_define_type value_type}}> newBeans)
    {
        if(_dataMap == null)
            return;
        foreach(var bean in newBeans)
        {
            _dataMap[bean.{{x.index_field.convention_name}}] = bean;
        }
        _dataList.Clear();
        _dataList.AddRange(_dataMap.Values);
    }
...
{{~end~}}
    ...
    {
        _dataMap.TryGetValue(key, out var res);
        return res;
    }
...
{{~end~}}
    ...
    public System.Type BeanType => typeof({{cs_define_type value_type}});
